...
def insert(cups, currentCupLabel, to_move):
  ...
  if position == len(cups) - 1:
    ...
    cups.append(to_move[0])
    cups.append(to_move[1])
    cups.append(to_move[2])
  else:
    cups.insert(position+1, to_move[0])
    cups.insert(position+2, to_move[1])
    cups.insert(position+3, to_move[2])
    # for x in range(len(to_move)):
...
cupOne = cups.index(1)
if cupOne+1 == len(cups):
  nextCup = 0
else:
  nextCup = cupOne+1
if nextCup+1 == len(cups):
  nextestCup = 0
else:
  nextestCup = nextCup+1

print(f'after 1,000,000 rounds, the cups after 1 are: {cups[nextCup]} and {cups[nextestCup]}, multiplied: {cups[nextCup] * cups[nextestCup]}')
